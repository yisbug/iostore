...
class Model {
  constructor(config) {
    const { state = {}, reducers = {}, helper = {} } = config;
    this.state = state;
    ...
    Object.keys(helper).forEach(name => {
      this.helper[name] = (...args) => {
        return helper[name].apply({ state: this.state, ...this.helper }, args);
      };
    });

    // 处理reducer
    if (this.reducers.dispatch) throw new Error('dispatch is now allowed in reducers.');
    if (this.reducers.helper) throw new Error('helper is now allowed in reducers.');
    this.methods = this.dispatch;
    Object.keys(this.reducers).forEach(type => {
      this.methods[type] = this.createMethod(type);
    });
    this.methods.helper = this.helper;
    ...
    this.useLoading = () => {
      const types = Object.keys(this.methods);
      for (let i = 0; i < types.length; i += 1) {
        if (this.methods[types[i]].loading) return true;
      }
      return false;
    };
  }
  ...
  createMethod(type) {
    const method = (...args) => {
      ...
      if (method.loading) {
        console.log('method.loading', method.loading);
        method.promise
